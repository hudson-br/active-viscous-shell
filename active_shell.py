...
class ActiveShell:
    ...
    def write(
        ...
        if activity:
            ...
            self.Qtilde = project(self.v_1n, self.V_thickness)
            self.Qtilde.rename("Qtilde", "Qtilde")
            self.output_file.write(self.Qtilde, i)

            self.small_q = project(self.v_2n, self.V_thickness)
            self.small_q.rename("q", "q")
            self.output_file.write(self.small_q, i)

            Q_tensor = (
                (1/6 + self.v_1n)*outer(self.a1,self.a1) +
                (1/6 - self.v_1n)*outer(self.a2,self.a2) +
                (self.v_2n)*outer(self.a1,self.a2) +
                (self.v_2n)*outer(self.a2,self.a1) +
                (0.)*outer(self.n0,self.n0)
                )
            Q_tensor = project(Q_tensor,self.V_tensor)
            # print(Q_tensor.ufl_shape)
            # Q_tensor = project(as_tensor([[1/6 + self.v_1n, self.v_2n, 0 ],[self.v_2n, 1/6 - self.v_1n, 0], [0 , 0, -1./3]]),self.V_tensor)
            Q_tensor.rename("Qtensor","Qtensor")
            self.output_file.write(Q_tensor,i)
    ...
    def set_functions_space(self):
        ...
        element = MixedElement([VectorElement(P2, dim=3), VectorElement(CR1, dim=2), VectorElement(P2, dim = 2), R])
        ...
        self.u_, self.beta_, self.nematic_, self.lbda_ = split(self.q_)
        self.u, self.beta, self.nematic, self.lbda = split(self.q)
        self.u_t, self.beta_t, self.nematic_t, self.lbda_t = split(self.q_t)

        self.u_1, self.u_2, self.u_3 = split(self.u_)
        ...
        self.V_tensor = TensorFunctionSpace(self.mesh, "CG", 1, shape=(3, 3))
        ...
        # Nematics
        element_nematic = MixedElement([P1,P1])
        self.V_nematic = FunctionSpace(self.mesh,element_nematic)

        # self.v_ = Function(self.V_nematic)
        self.v_n = Function(self.Q.sub(2).collapse())

        self.v_1, self.v_2 =  split(self.nematic_)
        self.v_1n, self.v_2n =  split(self.v_n)

        # self.dv_ = TestFunction(self.Q.sub(2).collapse())
        self.dv_1, self.dv_2 = split(self.nematic_t)
    ...
    def set_boundary_conditions(self):
        ...
        def poles(x, on_boundary):
            return near(x[2], 0.0, 1.0e-3) and near(x[0], 0.0, 1.0e-3)  and on_boundary
        ...
        bc_nematic = DirichletBC(self.Q.sub(2), Constant((0.0,0.0)), poles)
    ...
    def set_nematic_tensor(self):
        self.nematic_tensor = as_tensor([[1.0 / 6 + self.v_1, self.v_2], [self.v_2, 1.0 / 6 - self.v_1]])
        self.nematic_tensor_n = as_tensor([[1.0 / 6 + self.v_1n, self.v_2n], [self.v_2n, 1.0 / 6 - self.v_1n]])
    def set_energies(self):
        ...
        self.set_nematic_tensor()

        self.Q_tensor = self.nematic_tensor
        ...
        Q_alphabeta = as_tensor(
            (
                self.a0_contra[i, l] * self.a0_contra[j, m] * self.nematic_tensor[l, m]
        ...
        self.u_tangential = as_tensor([self.u_1,self.u_2])
        omega12 = 0.5*(self.d1(self.u_2)- self.d2(self.u_1))
        self.omega = as_tensor([[0,  omega12],[-omega12, 0]])
        self.corotational_term = as_tensor(self.omega[i,k]*self.nematic_tensor[k,j] + self.omega[j,k]*self.nematic_tensor[k,i],[i,j])


        self.N_reorientation = (
            - self.thickness 
            * self.gamma_ 
            * self.nu_ 
            * as_tensor(self.a0_contra[i, l] * self.a0_contra[j, m] 
            * ((self.nematic_tensor[l, m] - self.nematic_tensor_n[l, m])/self.dt),[i,j])
            - self.thickness 
            * self.gamma_ 
            * self.nu_ 
            * as_tensor(self.a0_contra[i, l] * self.a0_contra[j, m] 
            * self.corotational_term[l,m],[i,j])
            + 
            self.thickness 
            * self.gamma_ 
            * self.nu_ * self.nu_
            * as_tensor(self.a0_contra[i, l] * self.a0_contra[j, m] 
            * self.membrane_deformation()[l,m],[i,j])
            +
            self.thickness 
            * self.gamma_ 
            * self.nu_ 
            * as_tensor(self.a0_contra[i,j],[i,j])
            * self.chi_/3

        )

        self.N = self.N_active + self.N_passive + self.N_polymerization + self.N_reorientation
    ...
    def set_total_energy(self):
        ...
        self.nematicresidue = (\
            ((self.v_1 - self.v_1n)/self.dt)*self.dv_1 \
            + 0.5*(self.corotational_term[0,0] - self.corotational_term[1,1])* self.dv_1 \
            # + dot(self.grad_(self.v_1), self.u_tangential) * self.dv_1
            - 0.5*self.nu_ \
            * (self.membrane_deformation()[0,0] - self.membrane_deformation()[1,1]) * self.dv_1  \
            + (self.L_/self.gamma_)*dot(self.grad_(self.v_1), self.grad_(self.dv_1)) \
            + (self.chi_/self.gamma_)*self.v_1*self.dv_1  \
            + \
            ((self.v_2 - self.v_2n)/self.dt)*self.dv_2  \
            + (self.corotational_term[0,1])* self.dv_2 \
            # + dot(self.grad_(self.v_2), self.u_tangential) * self.dv_2
            - self.nu_ \
            *(self.membrane_deformation()[0,1]) * self.dv_2  \
            # *(self.membrane_deformation()[0,1] + self.membrane_deformation()[1,0]) * self.dv_2  \
            + (self.L_/self.gamma_)*dot(self.grad_(self.v_2), self.grad_(self.dv_2))  \
            + (self.chi_/self.gamma_)*self.v_2*self.dv_2  \
        )
        ...
        self.dPi = derivative(self.Pi, self.q_, self.q_t) + self.nematicresidue * sqrt(self.j0) * self.dx
    ...
    def evolution(self, dt):
        ...
        self.v_n.assign(self.q_.sub(2,True))
        self.v_1n, self.v_2n =  split(self.v_n)
    ...
    def mesh_refinement(self, control_type):
        with XDMFFile("mesh.xdmf") as ffile:
            ...
            ffile.write(self.mesh,XDMFFile.Encoding.ASCII)
            # ffile.write(self.mesh,XDMFFile.Encoding.HDF5)
    ...
    def adapt_and_interpolate(self):
        ...
        self.nematic_ = interpolate(self.q_.sub(2), self.Q.sub(2).collapse())
        ...
        # self.v_ = interpolate(self.v_, self.V_nematic)
        self.v_n = interpolate(self.v_n, self.Q.sub(2).collapse())
        # self.v_n = Function(self.Q.sub(2).collapse())
